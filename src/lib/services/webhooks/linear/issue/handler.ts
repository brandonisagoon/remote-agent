...
export type { IssueWebhookResult } from "../../../../../types/webhooks/linear/index.ts";
...
export async function handleIssueWebhook(input: {
  ...
  if (data.team?.key === config.agentTeamKey) {
    ...
    const receipt = await createWebhookReceipt(prisma, {
      ...
      eventType: "issue",
      trigger: "end",
      cubeIssueIdentifier: data.identifier,
    ...
    void dispatchEvent({
      ...
      event: { type: DispatchEventType.LinearIssueEndRequested, webhook },
    });
  ...
  void dispatchEvent({
    ...
    event: {
      type:
        trigger === "reflection"
          ? DispatchEventType.LinearIssueReflectionRequested
          : DispatchEventType.LinearIssueOrchestrationRequested,
      webhook,
    },
  });
  return {
