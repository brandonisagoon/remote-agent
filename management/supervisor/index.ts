...
/** The server's registration: bun running server.ts out of the deployment
    copy, pointed at the canonical config file. */
export function serverDefinition(input: {
  serviceName: string;
  appRoot: string;
  configFile: string;
  logFile: string;
  ...
  return {
    ...
    command: [bun, path.join(input.appRoot, "apps", "server", "server.ts")],
    ...
    environment: {
      ...
      // The server spawns provider CLIs; give it the package-manager dirs a
