...
export async function serviceStatus(): Promise<ManagementResult> {
  try {
    ...
    return {
      ok: response.ok,
      summary: response.ok ? "Server is running" : `Server returned ${response.status}`,
    };
  } catch (error) {
    return {
      ok: false,
...
export async function installUpdate(): Promise<ManagementResult> {
  ...
  const script = existsSync(config.deployScript)
    ? config.deployScript
    : path.join(sourceRoot(), "management", "deploy.ts");
...
/** Unregisters the server. Leaves state/ (database, keys, logs) and the
    cloudflared service alone — losing session registrations to a reinstall
...
export async function restartService(): Promise<ManagementResult> {
  try {
    ...
    await supervisor().restart(serverDefinition({
