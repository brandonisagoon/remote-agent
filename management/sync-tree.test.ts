...
describe("syncTree", () => {
  test("copies new and changed files and deletes removed ones", () => {
    write(from, "apps/server/app.ts", "new");
    write(from, "package.json", "{}");
    write(to, "apps/server/app.ts", "old");
    write(to, "apps/removed.ts", "gone");
    syncTree(from, to);
    expect(readFileSync(path.join(to, "apps/server/app.ts"), "utf8")).toBe("new");
    expect(readFileSync(path.join(to, "package.json"), "utf8")).toBe("{}");
    expect(existsSync(path.join(to, "apps/removed.ts"))).toBeFalse();
  });
  ...
  test("never copies .git or node_modules and preserves target-only build products", () => {
    ...
    write(from, "apps/server/app.ts", "code");
    write(to, "node_modules/dep/index.js", "installed dep");
    write(to, "generated/prisma/client.ts", "generated");
    syncTree(from, to);
    ...
    expect(readFileSync(path.join(to, "generated/prisma/client.ts"), "utf8")).toBe("generated");
  });
